{ lib, buildGoModule, fetchFromGitHub, fetchpatch, buildEnv, linkFarm
, makeWrapper, stdenv, addDriverRunpath, nix-update-script,

cmake, gitMinimal, clblast, libdrm, rocmPackages
, rocmGpuTargets ? rocmPackages.clr.gpuTargets or [ ], cudaPackages
, cudaArches ? cudaPackages.cudaFlags.realArches or [ ], darwin
, autoAddDriverRunpath, versionCheckHook,
...
let
  ...
  src = fetchFromGitHub {
    ...
    repo = "ollama";
    tag = "v${version}";
    hash = "sha256-gTQ4CYdMsYlOa2xn6Kwy5j5KY3WxgGixPZvRR+KNsL4=";
  ...
  cudaLibs =
    [ cudaPackages.cuda_cudart cudaPackages.libcublas cudaPackages.cuda_cccl ];
  ...
  wrapperOptions = [
    ...
    "--suffix LD_LIBRARY_PATH : '${
      lib.makeLibraryPath (map lib.getLib cudaLibs)
    }'"
  ];
...
in goBuild {
  ...
  buildInputs = lib.optionals enableRocm (rocmLibs ++ [ libdrm ])
  ...
  preBuild = let
    removeSMPrefix = str:
      let matched = builtins.match "sm_(.*)" str;
      in if matched == null then str else builtins.head matched;

    cudaArchitectures =
      builtins.concatStringsSep ";" (builtins.map removeSMPrefix cudaArches);
    rocmTargets = builtins.concatStringsSep ";" rocmGpuTargets;

    cmakeFlagsCudaArchitectures = lib.optionalString enableCuda
      "-DCMAKE_CUDA_ARCHITECTURES='${cudaArchitectures}'";
    cmakeFlagsRocmTargets =
      lib.optionalString enableRocm "-DAMDGPU_TARGETS='${rocmTargets}'";
  ...
  } // lib.optionalAttrs (!enableRocm && !enableCuda) {
  ...
  meta = {
    ...
    platforms = if (rocmRequested || cudaRequested) then
      platforms.linux
    else
      platforms.unix;
    mainProgram = "ollama";
